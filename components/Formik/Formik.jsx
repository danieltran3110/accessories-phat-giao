...
import RatingContext from '../RatingContext/RatingContext';
...
const ReviewSchema = Yup.object().shape({
  ...
  comment: Yup.string()
    ...
    .required('Required'),
  rating: Yup.number().required('Required').positive('Required').integer(),
});

export default function BoxFormik({ rating }) {
  const [checked, setChecked] = useState(false);
  let stars = useContext(RatingContext);
  ...
  return (
    <Formik
      initialValues={{ name: '', email: '', comment: '', rating: 0 }}
    ...
    >
      <Form className={styles.formInfo}>
        <div className={`${styles.displayGrid}`}>
          <div
            className={`${styles.fieldWrapper} ${rating && styles.gridColumn}`}
          >
            <ErrorMessage
              name='name'
        ...
        <div className={styles.fieldWrapper}>
          ...
          />
          <Field
            className={`${styles.input} ${styles.inputComment}`}
            rows='20'
            cols='50'
            name='comment'
            placeholder={'COMMENT'}
        ...
        <label
          className={`${styles.checkbox} ${styles.fontBold}`}
          htmlFor='agreePolicy'
        >
          {dataPolicy.agreePolicy}
        ...
        <label
          className={`${styles.checkbox} ${styles.fontBigger}`}
          htmlFor='agreeData'
        >
          {dataPolicy.agreeData}{' '}
          <abbr title='You need to accept this checkbox.'>*</abbr>
          <input
            id='agreeData'
            type='checkbox'
          />
