  ...
  return (
    <div className={styles.listPosts}>
      <div className={styles.firstPost}>
        ...
        <p className={styles.author}><span>POSTED BY</span> {dataFirstPost.authorPost}</p>
      ...
      {currentItems.map((_data, index) =>
        <div className={styles.wrapperPosts} key={index}>
          ...
          <p className={styles.title}>{_data.titlePost}
            {(!(_data.src) && !(_data.urlVideo)) && <ComponentDay _data={_data} />}
          </p>
          <p className={styles.author}><span>POSTED BY</span> {_data.authorPost}</p>
          {_data.content && <p className={styles.content}>{_data.content}</p>}
          <div className={`${stylesGlobal.button2} ${stylesGlobal.normal} ${stylesGlobal.BGCBlack}`}>LEARN MORE</div>
        </div>,
      )}
    </div>
  );
